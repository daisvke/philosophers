...
int	ph_hold_a_fork(t_env *env, t_philo *philo, size_t id, bool next_fork)
{
	...
	if (ph_print_msg(env, philo, MSG_TAKE_FORK) == ERROR \
		|| ph_pthread_mutex_lock(env, &env->forks[id]) == ERROR)
		return (ERROR);
	if (next_fork)
	{
		if (ph_gettime(env, &curr_time) == ERROR)
			return (ERROR);
		ph_pthread_mutex_lock(env, &env->locks[LK_LAST_MEAL_TIME]);
		philo->last_meal_time = curr_time;
		ph_pthread_mutex_unlock(env, &env->locks[LK_LAST_MEAL_TIME]);
	}
	return (SUCCESS);
}
...
int	ph_drop_forks(t_env *env, size_t *fork_ids)
{
	...
	fork_2 = fork_ids[1];
	error_occured = false;
	if (ph_pthread_mutex_unlock(env, &env->forks[fork_1]) != SUCCESS)
		error_occured = true;
	if (ph_pthread_mutex_unlock(env, &env->forks[fork_2]) != SUCCESS)
		error_occured = true;
...
int	ph_wait_until_eating(t_env *env, t_philo *philo, size_t *fork_ids)
{
	...
	if (env->philo_died == false)
	{
		...
		{
			ph_pthread_mutex_unlock(env, &env->forks[fork_1]);
			ph_pthread_mutex_unlock(env, &env->forks[fork_2]);
			return (ERROR);
		}
	}
	else
